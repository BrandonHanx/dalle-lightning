...
from pl_dalle.models.vqgan import EMAVQGAN, VQGAN, GumbelVQGAN
from pl_dalle.models.vqvae import EMAVQVAE, VQVAE, GumbelVQVAE
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--log_dir", type=str, default="results/", help="path to save logs"
    )
    parser.add_argument(
        "--backup_dir",
        type=str,
    ...
    parser.add_argument(
        "--ckpt_path",
        type=str,
        default="results/checkpoints/last.ckpt",
    ...
    parser.add_argument(
        "--refresh_rate", type=int, default=1, help="progress bar refresh rate"
    )
    parser.add_argument(
        "--precision", type=int, default=32, help="precision for training"
    )
    ...
    parser.add_argument(
        "--use_tpus", action="store_true", default=False, help="using tpu"
    )
    ...
    parser.add_argument("--tpus", type=int, default=8, help="number of tpus")
    parser.add_argument(
        "--num_sanity_val_steps", type=int, default=0, help="num_sanity_val_steps"
    )

    parser.add_argument(
        "--learning_rate", default=4.5e-6, type=float, help="base learning rate"
    )
    parser.add_argument(
        "--lr_decay", action="store_true", default=False, help="use learning rate decay"
    )
    ...
    parser.add_argument("--test", action="store_true", default=False, help="test run")
    parser.add_argument("--debug", action="store_true", default=False, help="debug run")
    parser.add_argument(
        "--xla_stat",
        action="store_true",
        default=False,
    ...
    parser.add_argument(
        "--web_dataset", action="store_true", default=False, help="enable web_dataset"
    )
    parser.add_argument(
        "--dataset_size",
        nargs="+",
        type=int,
        default=[260490, 32528],
    ...
    parser.add_argument("--num_tokens", type=int, default=1024, help="codebook size")
    parser.add_argument(
        "--double_z", type=bool, default=False, help="double z for encoder"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out_channels", type=int, default=3, help="output image channel"
    )
    parser.add_argument(
        "--hidden_dim", type=int, default=128, help="hidden dimension init size"
    )
    parser.add_argument(
        "--ch_mult",
        nargs="+",
    ...
    parser.add_argument(
        "--quant_ema_decay", type=float, default=0.99, help="quantizer ema decay"
    )
    parser.add_argument(
        "--quant_ema_eps", type=float, default=1e-5, help="quantizer ema epsilon"
    )

    # vqvae2 specialized options
    parser.add_argument("--num_res_ch", type=int, default=32, help="model settings")
    parser.add_argument(
        "--strides", nargs="+", type=int, default=[8, 2], help="vqvae2 strides"
    )

    # loss configuration
    parser.add_argument("--smooth_l1_loss", dest="smooth_l1_loss", action="store_true")
    parser.add_argument(
        "--kl_loss_weight", type=float, default=1e-8, help="KL loss weight"
    )
    parser.add_argument(
        "--disc_conditional", type=bool, default=False, help="lossconfig"
    )
    parser.add_argument("--disc_in_channels", type=int, default=3, help="lossconfig")
    parser.add_argument("--disc_start", type=int, default=10001, help="lossconfig")
    parser.add_argument("--disc_weight", type=float, default=0.8, help="lossconfig")
    parser.add_argument("--codebook_weight", type=float, default=1.0, help="lossconfig")
    ...
    # Appedn path
    args.train_dir = os.path.join(args.root, args.train_dir)
    ...
    if args.backup:
        ...
        if len(glob.glob(os.path.join(args.backup_dir, "*.ckpt"))) != 0:
            ...
            if args.resume:
                print(
                    "Setting default ckpt to {}. If this is unexpected behavior, remove {}".format(
                        ckpt_path, ckpt_path
                    )
                )

    if args.wandb:
        logger = pl.loggers.wandb.WandbLogger(project="vqvae", log_model=False)
        # logger.watch(model)
    else:
    ...
    if args.log_images:
        trainer.callbacks.append(
            ReconstructedImageLogger(
                every_n_steps=args.image_log_steps,
                use_wandb=args.wandb,
                multi_optim=args.multi_optim,
            )
        )

    print(
        "Setting batch size: {} learning rate: {:.2e}".format(
